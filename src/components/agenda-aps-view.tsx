...
export function AgendaApsView({ initialAgenda }: { initialAgenda?: ApsAgenda }) {
  ...
  return <div className="mx-auto max-w-6xl">
    <section className="agenda-banner mb-4 rounded-2xl p-4 text-white shadow-lg shadow-black/10">
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(320px,.9fr)] lg:items-center">
        <div>
          ...
          <p className="mt-1.5 text-xs text-white/80">{agenda ? `${agenda.filename} · Actualizado el ${formatDateTime(agenda.updatedAt)} por ${agenda.updatedBy}` : "Aún no hay Agenda APS vigente."}</p>
          {agenda ? <a className="mt-1.5 inline-flex items-center gap-1 text-[11px] font-semibold text-white/80 underline-offset-2 hover:text-white hover:underline" href="/api/agenda-aps?download=1"><Download size={13} /> Descargar Excel</a> : null}
        </div>
